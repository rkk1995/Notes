...
<iframe width="560" height="315" src="https://www.youtube.com/embed/FU4WlwfS3G0" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

-------

## Problem Statement / Requirement Clarification

![](image/Rate Limiting (local and distributed)/1634777600698.png)
...
It always makes sense to start/mention the simple solution and evolve the solution. Let's implement a solution for a single server.

![](image/Rate Limiting (local and distributed)/1634778346602.png)
...
- Allow request method has one argument - number of tokens that represent a cost of the operation; usually, the cost is equal to 1, but it may be a larger value as well.
...
![](image/Rate Limiting (local and distributed)/1634779616875.png)

### Leaky Bucket

![](image/Rate Limiting (local and distributed)/1634779418555.png)

Leaky Bucket algorithm is opposite of Token Bucket algorithm. Leaky Bucket allows requests at a constant rate, the rate of the leak. There is no concept of a token, the requests go into the bucket and flow out at a constant rate. If there is no space in the bucket for the request, the request is discarded.

![](image/Rate Limiting (local and distributed)/1634779512562.png)

## Object Oriented Design

![](image/Rate Limiting (local and distributed)/1634779795899.png)
...
- RetrieveJobScheduler class implements JobScheduler interface.
  - Its responsibility is to instantiate, start and stop the scheduler, and to run retrieve rules task.
...
- Should we give 4 divided by 3? And the answer is 4. Each bucket should have 4 tokens initially.
...
So, communication between hosts is the key. Let's see how this communication can be implemented. By the way, ideas we will discuss next are applicable not only for rate limiting solution, but many other distributed systems that require data sharing between all hosts in a cluster in a real time.

### Message Broadcasting

![](image/Rate Limiting (local and distributed)/1634780553740.png)
...
How do hosts talk to each other. We have two options here: TCP and UDP.
...
![](image/Rate Limiting (local and distributed)/1634780963428.png)
...
- What are the pros for the second approach?
...
![](image/Rate Limiting (local and distributed)/1634781311973.png)
...
![](image/Rate Limiting (local and distributed)/1634781348245.png)
...
- This is a distributed cache option we discussed above. But the drawback of this approach is that it increases latency and operational cost.
